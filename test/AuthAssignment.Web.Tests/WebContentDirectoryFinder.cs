...
public static class WebContentDirectoryFinder
{
    public static string? CalculateContentRootFolder()
    {
        var domainAssemblyDirectoryPath =
            Path.GetDirectoryName(typeof(AuthAssignmentDomainModule).Assembly.Location);
        if (domainAssemblyDirectoryPath == null)
        {
            throw new Exception(
                $"Could not find location of {typeof(AuthAssignmentDomainModule).Assembly.FullName} assembly!");
        }
        ...
        {
            while (!DirectoryContains(directoryInfo.FullName, "AuthAssignment.Web.csproj", SearchOption.AllDirectories))
            {
            ...
            var webProject = Directory.GetFiles(directoryInfo.FullName, string.Empty, SearchOption.AllDirectories)
                .First(filePath => string.Equals(Path.GetFileName(filePath), "AuthAssignment.Web.csproj"));

            return Path.GetDirectoryName(webProject);
        }

        while (!DirectoryContains(directoryInfo.FullName, "AuthAssignment.sln"))
        {
            directoryInfo = directoryInfo.Parent ?? throw new Exception("Could not find content root folder!");
        }

        var webFolder = Path.Combine(directoryInfo.FullName, $"src{Path.DirectorySeparatorChar}AuthAssignment.Web");
